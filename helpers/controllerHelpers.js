...
function unpair(cat) {
  cat.unpair();
  moveAnimate(
    `.${PARTICIPANT}.${cat.name}`,
    `.${PARTICIPANT_CONTAINER}.${cat.name}`
  );
}
...
function pair(human, cat) {
  ...
  moveAnimate(
    `.${PARTICIPANT}.${cat.name}`,
    `.${PARTICIPANT_CONTAINER}.${human.name} .${CAT_ADOPTION_CONTAINER}`
  );
}
...
function tryCat(human, catIdx) {
  ...
  $($(`.${PARTICIPANT}.${human.name}`).find(`.${PREFERENCE}`)[catIdx]).addClass(
    TRIED
  );
}
